...
function coseToAlgorithm(cose) {
	const alg = {};
	if (algMap[cose.alg]) {
		alg.name = algMap[cose.alg];
	}
	if (algHashes[cose.alg]) {
		alg.hash = algHashes[cose.alg];
	}
	if (cose.crv) {
		alg.namedCurve = cose.crv;
	}
	return alg;
}
...
function jwkToAlgorithm(jwk) {
	const alg = {};
	if (algMapJwkToSubtle[jwk.alg]) {
		alg.name = algMapJwkToSubtle[jwk.alg];
	}
	if (jwk.crv) {
...
class Key {
	...
	async fromCose(cose) {
		...
		// Store algorithm
		this._alg = coseToAlgorithm(retKey);

		// Convert cose algorithm identifier to jwk algorithm name
		retKey.alg = algMapJwk[retKey.alg];
