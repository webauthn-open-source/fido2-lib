...
class PublicKey {
	...
	async fromPem(pem, hashName) {
		...
		if (keyInfo.algorithm.algorithmId === "1.2.840.10045.2.1") {
			...
			// Handle Ed448
		} else if (keyInfo.algorithm.algorithmId === "1.3.101.113") {
	...
	setAlgorithm(algorithmInput) {
		...
		if (algorithmInput.name) {
			...
			} else if (algorithmInput.alg === "EdDSA" && algorithmInput.crv) {
				// For EdDSA, the WebCrypto algorithm name matches the curve name (e.g. "Ed25519", "Ed448")
				algorithmOutput.name = algorithmInput.crv;
			}
		}

		// EdDSA algorithms (Ed25519, Ed448) do not use a separate hash or namedCurve
		const isEdDSA = algorithmOutput.name === "Ed25519" || algorithmOutput.name === "Ed448";
		...
		// Try to extract namedCurve if not already present (not applicable for EdDSA)
		if (!isEdDSA) {
