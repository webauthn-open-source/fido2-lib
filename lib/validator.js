...
async function validateExpectations() {
  ...
  if (req.size !== exp.size) {
    throw new Error(
      `wrong number of expectations: should have ${req.size} but got ${exp.size}`
    );
  }
  ...
  if (exp.has("challenge")) {
    ...
    if (typeof challenge !== "string") {
      throw new Error(
        "expected challenge should be of type String, got: " + typeof challenge
      );
    }
...
function validateCreateRequest() {
  ...
  if (!(req.rawId instanceof ArrayBuffer) && !(req.id instanceof ArrayBuffer)) {
    throw new TypeError(
      "expected 'id' or 'rawId' field of request to be ArrayBuffer, got rawId " +
        typeof req.rawId +
        " and id " +
        typeof req.id
    );
  ...
  if (typeof req.response !== "object") {
    throw new TypeError(
      "expected 'response' field of request to be Object, got " +
        typeof req.response
    );
  }

  if (
    typeof req.response.attestationObject !== "string" &&
  ...
  if (
    typeof req.response.clientDataJSON !== "string" &&
...
function validateAssertionResponse() {
  ...
  if (
    typeof req.response.authenticatorData !== "string" &&
  ...
  if (
    typeof req.response.signature !== "string" &&
  ...
  if (
    typeof req.response.userHandle !== "string" &&
...
async function validateCreateType() {
  ...
  if (type !== "webauthn.create") {
    throw new Error(
      "clientData type should be 'webauthn.create', got: " + type
    );
  }
...
async function validateTokenBinding() {
  ...
  if (typeof tokenBinding === "object") {
    if (
      ...
      throw new Error(
        "tokenBinding status should be 'not-supported' or 'supported', got: " +
          tokenBinding.status
      );
    }
...
async function validateRpIdHash() {
  ...
  var createdHash = new Uint8Array(
    crypto
      .createHash("sha256")
      .update(domain)
      .digest()
...
async function validateCredId() {
  ...
  if (typeof credIdLen !== "number") {
    throw new Error(
      "authnrData credIdLen should be number, got " + typeof credIdLen
    );
  }
...
async function validatePublicKey() {
  ...
  if (!(cbor instanceof ArrayBuffer)) {
    throw new Error(
      "authnrData credentialPublicKeyCose isn't of type ArrayBuffer"
    );
  }
  ...
  if (typeof jwk.kty !== "string") {
    throw new Error(
      "authnrData credentialPublicKeyJwk.kty isn't of type String"
    );
  }

  if (typeof jwk.alg !== "string") {
    throw new Error(
      "authnrData credentialPublicKeyJwk.alg isn't of type String"
    );
  }
  ...
  switch (jwk.kty) {
    case "EC":
      if (typeof jwk.crv !== "string") {
        throw new Error(
          "authnrData credentialPublicKeyJwk.crv isn't of type String"
        );
      }
      break;
    case "RSA":
      if (typeof jwk.n !== "string") {
        throw new Error(
          "authnrData credentialPublicKeyJwk.n isn't of type String"
        );
      }

      if (typeof jwk.e !== "string") {
        throw new Error(
          "authnrData credentialPublicKeyJwk.e isn't of type String"
        );
      }
      break;
    default:
  ...
  if (journal.size !== clientData.size + authnrData.size) {
    throw new Error(
      `internal audit failed: ${
        journal.size
      } fields checked; expected ${clientData.size + authnrData.size}`
    );
  }
