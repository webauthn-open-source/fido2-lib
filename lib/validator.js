		...
		if (allowCredentials != null) {
			if (!Array.isArray(allowCredentials)) {
				...
				for (const index in allowCredentials) {
					...
					if (allowCredentials[index].transports != null && ["usb", "nfc", "ble", "internal"].includes(allowCredentials[index].transports)) {
						throw new Error("expected type of allowCredentials[" + index + "] to be string with value 'usb', 'nfc', 'ble', 'internal' or null");
					}
				}
			}
		}
...
async function validateId() {
	...
	if (allowCredentials !== undefined) {
		if (!allowCredentials.some(cred => bufEqual(rawId, cred.id))) {
			throw new Error("id does not match any value in allowCredentials");
		}
	}
