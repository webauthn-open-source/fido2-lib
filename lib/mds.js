...
class MdsCollection {
	...
	validate() {
		...
		this.unvalidatedEntryList.forEach((entry) => {
			...
			const tocEntryRaw = Buffer.from(JSON.stringify(tocEntry.metadataStatement)).toString("base64")
			const tocHash = crypto.createHash("sha256");
			tocHash.update(tocEntryRaw);
			var tocEntryHash = tocHash.digest();

			tocEntryHash = coerceToArrayBuffer(entryHash, "MDS TOC entry hash");
			...
			if (Array.isArray(entryId)) {
				// U2F array of IDs
				entryId.forEach((id) => {
					this.entryList.set(id.replace(/-/g, ""), newEntry);
				});
			} else {
				// UAF and FIDO2
				this.entryList.set(entryId.replace(/-/g, ""), newEntry);
			}
		});
	}
	...
	findEntry(id) {
		if (id instanceof ArrayBuffer) {
			id = coerceToBase64(id, "MDS entry id");
		}
		...
		return this.entryList.get(Buffer.from(id, "base64").toString("hex").replace(/-/g, "")) || null;
	}
}
