...
class MdsCollection {
	...
	async addToc(tocStr, rootCert, crls) {
		...
		// check for MDS v2
		if (this.toc.entries.some(entry => !entry.metadataStatement)) console.log("[DEPRECATION WARNING] FIDO MDS v2 will be removed in October 2022. Please update to MDS v3!");
	...
	addEntry(entryStr) {
		...
		if (newEntry.metadataStatement) {
			...
			entryStr = Buffer.from(JSON.stringify(newEntry).replace(/[\u{0080}-\u{FFFF}]/gu,"")).toString("base64");
		}
	...
	validate() {
		...
		this.unvalidatedEntryList.forEach((entry) => {
			...
			if (tocEntry.hash) {
				...
				tocHash.update(Buffer.from(JSON.stringify(tocEntry.metadataStatement).replace(/[\u{0080}-\u{FFFF}]/gu,"")).toString("base64"));
	...
	findEntry(id) {
		...
		return this.entryList.get(id) || this.entryList.get(Buffer.from(id, "base64").toString("hex").replace(/-/g, "")) || null;
	}
}
