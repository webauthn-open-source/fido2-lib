	...
	validate() {
		...
		this.unvalidatedEntryList.forEach((entry) => {
			...
			if (Array.isArray(entryId)) {
				// U2F array of IDs
				entryId.forEach((id) => {
					this.entryList.set(tocEntry.metadataStatement ? id : id.replace(/-/g, ""), newEntry);
				});
			} else {
				// UAF and FIDO2
				this.entryList.set(tocEntry.metadataStatement ? entryId : entryId.replace(/-/g, ""), newEntry);
			}
		});
	}
