...
const { CertManager } = require("./certUtils");
...
class MdsEntry {
  ...
  constructor(mdsEntry, tocEntry) {
    ...
    if (!Array.isArray(this.attestationTypes))
      throw new Error(
        "expected attestationTypes to be Array, got: " + this.attestationTypes
      );
    this.attestationTypes = this.attestationTypes.map(att =>
      attestationTypeToStr(att)
    );
    ...
    this.publicKeyAlgAndEncoding = pkAlgAndEncodingToStr(
      this.publicKeyAlgAndEncoding
    );
    function pkAlgAndEncodingToStr(pkalg) {
      switch (pkalg) {
        ...
        default:
          throw new Error(
            "unknown public key algorithm and encoding: " + pkalg
          );
      }
    }
    ...
    function tcDisplayToArr(tcd) {
      ...
      if (tcd & 0xffe0)
        throw new Error(
          ("unknown transaction confirmation display flags: " + tcd) & 0xffe0
        );
      return ret;
    }
    ...
    function uvDetailsToSet(uvList) {
      ...
      if (!Array.isArray(uvList))
        throw new Error(
          "expected userVerificationDetails to be an Array, got: " + uvList
        );
      uvList.forEach(uv => {
        if (!Array.isArray(uv))
...
class MdsCollection {
  ...
  async addToc(tocStr, rootCert, crls) {
    ...
    var parsedJws;
    try {
      parsedJws = await jose.JWS.createVerify().verify(tocStr, {
        allowEmbeddedKey: true
      });
      this.toc = JSON.parse(
        ab2str(coerceToArrayBuffer(parsedJws.payload, "MDS TOC payload"))
      );
    } catch (e) {
  ...
  validate() {
    ...
    this.unvalidatedEntryList.forEach(entry => {
      ...
      if (tocEntry.length !== 1) {
        throw new Error(
          `found the wrong number of TOC entries for '${entryId}': ${tocEntry.length}`
        );
      }
      tocEntry = tocEntry[0];
      ...
      var tocEntryHash = coerceToArrayBuffer(
        tocEntry.hash,
        "MDS TOC entry hash"
      );
      if (!abEqual(entryHash, tocEntryHash)) {
        throw new Error(
          "MDS entry hash did not match corresponding hash in MDS TOC"
        );
      }
