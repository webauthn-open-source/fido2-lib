...
function decodeValue(valueBlock) {
	...
	if (valueBlock && typeof valueBlock === "object" && "valueBlock" in valueBlock && valueBlock.valueBlock) {
		...
		// Check if it's a Constructed type (has value array)
		if (Array.isArray(innerBlock.value) && innerBlock.value.length > 0) {
			return decodeValue(innerBlock.value[0]);
		}
		
		// Check if it has value property as string first (BmpString, Utf8String)
		...
		// Check if it has valueHex (OctetString)
	...
	// Check for LocalIntegerValueBlock (has valueDec getter)
	if ("valueDec" in valueBlock) {
		return valueBlock.valueDec;
	}
	
	// Check for LocalBitStringValueBlock (has unusedBits and valueHex)
	...
	// Check for LocalOctetStringValueBlock (has isConstructed and valueHex but no unusedBits)
	...
	// Check for LocalConstructedValueBlock (has value array)
	if (typeof valueBlock === "object" && "value" in valueBlock && Array.isArray(valueBlock.value)) {
		return valueBlock.value.map((v) => decodeValue(v));
	}
	
	// Check for string value blocks (LocalSimpleStringValueBlock, LocalUtf8StringValueBlock, LocalBmpStringValueBlock)
	// These have a value property that's a string
