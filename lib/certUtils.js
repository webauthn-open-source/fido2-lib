...
const { isPem, pemToBase64, coerceToArrayBuffer, ab2str } = require("./utils");
    ...
    for (let ext of this._cert.extensions) {
      ...
      try {
        ...
        if (ext.critical === false) {
          this.warning.set(
            "x509-extension-error",
            ext.extnID + ": " + err.message
          );
          continue;
        } else {
          throw err;
...
function decodeValue(valueBlock) {
  ...
  switch (blockType) {
    ...
    default:
      throw new TypeError(
        "unknown value type when decoding certificate: " + blockType
      );
  }
}
...
function decodeU2FTransportType(u2fRawTransports) {
  if (
    !(u2fRawTransports instanceof ArrayBuffer) ||
...
function decodeFidoAaguid(value) {
  ...
  if (value.byteLength !== 18) {
    throw new Error(
      "AAGUID ASN.1 was wrong size. Should be 18, got " + value.byteLength
    );
  }
...
function decodeCertificatePolicies(value) {
  if (value && Array.isArray(value.certificatePolicies)) {
    value = value.certificatePolicies.map(() =>
      resolveOid(
        value.certificatePolicies[0].policyIdentifier,
        value.certificatePolicies[0].policyQualifiers
      )
    );
  }
...
function decodePolicyQualifiers(value) {
  if (value && Array.isArray(value)) {
    value = value.map(qual =>
      resolveOid(qual.policyQualifierId, qual.qualifier)
    );
  }
