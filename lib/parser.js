...
function parseExpectations(exp) {
	...
	let expRpId = exp.rpId;
	// origin
	if (!expRpId && exp.origin) {
		console.log("The 'origin' parameter is deprecated. It's suggested to update your code to use 'rpId' instead."); // eslint-disable-line no-console
		...
		expRpId = new URL(origin).hostname;
	}

	// rpId
	if (expRpId) {
		if (typeof expRpId !== "string") {
			throw new TypeError("expected 'rpId' should be string, got " + typeof expRpId);
		}

		let rpId = checkRpId(expRpId);
		ret.set("rpId", rpId);
	}
