...
function parseClientData(clientDataJSON) {
    ...
    try {
        ...
        throw new Error("couldn't parse clientDataJson");
    }

    var ret = new Map([
        ["challenge", parsed.challenge],
        ["clientExtensions", parsed.clientExtensions],
...
function parseAttestationObject(attestationObject) {
    ...
    var ret = new Map([
        ["fmt", parsed[0].fmt],
        // assign empty object when "none" because of https://github.com/hildjj/node-cbor/issues/64
        ["attStmt", (parsed[0].fmt === "none")?{}:parsed[0].attStmt],
        // return raw buffer for future signature verification
        ["rawAuthData", buf2ab(parsed[0].authData)],
        // parse authData
...
function parseAuthenticatorData(authnrDataArrayBuffer) {
    ...
    if (!(authnrDataArrayBuffer instanceof ArrayBuffer)) {
        throw new TypeError("couldn't convert 'authnrDataArrayBuffer' to ArrayBuffer");
    }
