...
function parseExpectations(exp) {
  ...
  if (exp.prevCounter !== undefined) {
    if (typeof exp.prevCounter !== "number") {
      throw new TypeError(
        "expected 'prevCounter' should be Number, got " + typeof exp.prevCounter
      );
    }
  ...
  if (exp.publicKey) {
    if (typeof exp.publicKey !== "string") {
      throw new TypeError(
        "expected 'publicKey' should be String, got " + typeof exp.publicKey
      );
    }
...
function parseClientResponse(msg) {
  ...
  var clientDataJSON = coerceToArrayBuffer(
...
function parseAuthenticatorData(authnrDataArrayBuffer) {
  ...
  authnrDataArrayBuffer = coerceToArrayBuffer(
    authnrDataArrayBuffer,
    "authnrDataArrayBuffer"
  );
  ...
  if (attestation) {
    ...
    offset += credIdLen;
    var credentialPublicKeyCose = authnrDataBuf.buffer.slice(
      offset,
      authnrDataBuf.buffer.byteLength
    );
...
function parseAuthnrAssertionResponse(msg) {
  ...
  if (msg.response.userHandle !== undefined) {
    userHandle = coerceToArrayBuffer(
      msg.response.userHandle,
      "response.userHandle"
    );
