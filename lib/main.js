...
const { Fido2AttestationResult, Fido2AssertionResult } = require("./response");
const { coerceToArrayBuffer, abToBuf } = require("./utils");
const { MdsCollection } = require("./mds");
...
class Fido2Lib {
  ...
  constructor(opts) {
    ...
    this.config.authenticatorRequireResidentKey =
      opts.authenticatorRequireResidentKey;
    ...
    this.config.authenticatorUserVerification =
      opts.authenticatorUserVerification;
    if (
  ...
  async attestationOptions(opts) {
    ...
    var extensions = createExtensions.call(
      this,
      "attestation",
...
function createExtensions(type, extObj) {
  ...
  for (let extension of enabledExtensions) {
    let extVal = this.generateExtensionOptions(
      extension,
      type,
