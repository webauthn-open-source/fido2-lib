...
class Fido2Lib {
    ...
    */
     ...
     * @see  MdsCollection
     */
    static createMdsCollection() {
        return new MdsCollection();
    }

    /**
     * Sets the global MDS collection that will be used for {@link findMdsEntry}
     * @param {MdsCollection} mdsCollection The MDS collection that will be used
     * @see  MdsCollection
     */
    static setMdsCollection(mdsCollection) {
        ...
        globalMdsCollection = mdsCollection;
    }

    /**
     * Sets the global MDS collection to `null`. Mostly used for testing.
     */
    static clearMdsCollection() {
        globalMdsCollection = null;
    }

    /**
     * Returns a MDS entry for the specified id, or `null` if none is found. The
     * lookup is done by calling {@link MdsCollection#findEntry} on the current global
     * MDS collection. If the global MDS collection hasn't been specified using
     * {@link setMdsCollection}, an `Error` will be thrown.
     * @param  {String|ArrayBuffer} id The authenticator id to look up metadata for
     * @return {MdsEntry|null}    Returns a {@link MdsEntry} for the entry that was found
     * or `null` if no entry was found.
     * @see  MdsCollection
     */
    static findMdsEntry(id) {
        if (!(globalMdsCollection instanceof MdsCollection)) {
            throw new Error("must set MDS collection before attempting to find an MDS entry");
        }

        return globalMdsCollection.findEntry(id);
    }
