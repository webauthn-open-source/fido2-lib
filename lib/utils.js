...
function checkRpId(rpId) {
	...
	var isValidUrl = true;
	try {
		rpIdUrl = new URL(rpId);
	} catch (err) {
		isValidUrl = false;
	}

	if (isValidUrl && rpIdUrl.protocol !== "https:") {
		throw new Error("rpId should be https");
	}

	if (!psl.isValid(rpId) && !isLocalhost && !isValidUrl) {
		throw new Error("rpId is not a valid eTLD+1");
	}
