...
async function verifySignature(publicKey, expectedSignature, data, hashName) {
	...
	// EdDSA algorithms (Ed25519, Ed448) do not use a separate hash algorithm
	const isEdDSA = alg.name === "Ed25519" || alg.name === "Ed448";

	// Use supplied hashName only for algorithms that rely on an external hash
	if (hashName && !isEdDSA) {
		alg.hash = {
			name: hashName,
		};
	...
	// Ensure no hash property is present for EdDSA algorithms
	if (isEdDSA && Object.prototype.hasOwnProperty.call(alg, "hash")) {
		delete alg.hash;
	}
