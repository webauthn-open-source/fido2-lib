...
describe("parseExpectations", function() {
  ...
  it("throws if flags is something other than Array or Set", function() {
    ...
    assert.throws(
      ...
      TypeError,
      "expected flags to be an Array or a Set, got: string"
    );
  });
  ...
  it("adds userHandle to map when it exists", function() {
    ...
    var expectedUserHandle = new Uint8Array([0x61, 0x6b]).buffer;

    assert.isTrue(
      abEqual(userHandle, expectedUserHandle),
      "userHandle has correct value"
    );
  });
