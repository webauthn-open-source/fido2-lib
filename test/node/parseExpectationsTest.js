...
import { coerceToArrayBuffer, abEqual } from "../../lib/common/utils.js";
...
describe("parseExpectations", function() {
	...
	it("coerces base64 challenge to base64url", function() {
		...
		let base64UrlChallenge = "4BS1YJKRCeCVoLdfG/b66BuSQ+I2n34WsLFvy62fpIVFjrm32/tFRQixX9U8EBVTriTkreAp+1nDvYboRK9WFg";
		let ret = parser.parseExpectations(exp);
		assert.instanceOf(ret, Map);
		assert.strictEqual(ret.size, 2);
		assert.strictEqual(ret.get("origin"), exp.origin);
	...
	it("adds allowCredentials to map when it exists", function() {
		...
		allowCredentials[0].id = coerceToArrayBuffer(allowCredentials[0].id, "allowCredentials");
