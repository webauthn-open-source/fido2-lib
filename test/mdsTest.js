...
const { MdsEntry, MdsCollection } = require("../lib/mds");
const chai = require("chai");
var chaiAsPromised = require("chai-as-promised");
chai.use(chaiAsPromised);
var assert = chai.assert;
const h = require("fido2-helpers");
const { str2ab, coerceToBase64Url } = require("../lib/utils");
...
describe("MdsCollection", function() {
  ...
  it("throws if name is empty string", function() {
    assert.throws(
      ...
      Error,
      "expected 'collectionName' to be non-empty string, got: "
    );
  });
  ...
  describe("addToc", function() {
    ...
    it("rejects if no jwk provided", function() {
      return assert.isRejected(
        mc.addToc(undefined),
        Error,
        "expected MDS TOC to be non-empty string"
      );
    });
    ...
    it("rejects if TOC is junk string", function() {
      ...
      return assert.isRejected(
        mc.addToc(toc),
        Error,
        "could not parse and validate MDS TOC: Unexpected token � in JSON at position 0"
      );
    });
    ...
    it("rejects if TOC header is missing alg", function() {
      ...
      jwtHeader = coerceToBase64Url(
        str2ab(JSON.stringify(jwtHeader)),
        "JWT header"
      );
      var jwtBody =
        "sL39APyTmisrjh11vghaqNfuruLQmCfR0c1ryKtaQ81jkEhNa5u9xLTnkibvXC9YpzBLFwWEZ3k9CR_sxzm_pWYbBOtKxeZu9z2GT8b6QW4iQvRlyumCT3oENx_8401r";
      ...
      return assert.isRejected(
        mc.addToc(toc),
        Error,
        "could not parse and validate MDS TOC: Algorithm not allowed: undefined"
      );
    });
    ...
    it("rejects if TOC header is missing typ", function() {
      ...
      return assert.isRejected(
        mc.addToc(toc),
        Error,
        "could not parse and validate MDS TOC: no key found"
      );
    });
    ...
    it("parses MDS1 TOC", async function() {
      var toc = await mc.addToc(h.mds.mds1TocJwt, h.mds.mdsRootCert, [
        h.mds.mdsRootCrl,
        h.mds.ca1Crl
      ]);
      assert.isObject(toc);
    ...
    it("parses MDS2 TOC", async function() {
      var toc = await mc.addToc(h.mds.mds2TocJwt, h.mds.mdsRootCert, [
        h.mds.mdsRootCrl,
        h.mds.ca1Crl
      ]);
      assert.isObject(toc);
    ...
    it("works with no CRLs", function() {
      return mc.addToc(h.mds.mds1TocJwt, h.mds.mdsRootCert);
    });
    ...
    it("throws on bad cert chain", function() {
      return assert.isRejected(
        mc.addToc(h.mds.mds2TocJwt, [h.certs.yubicoRoot]),
        Error,
  ...
  describe("addEntry", function() {
    ...
    it("throws on invalid jwk", function() {
      assert.throws(
        ...
        Error,
        "expected MDS entry to be non-empty string"
      );
    });
  ...
  describe("validate", function() {
    ...
    it("adds MDS1 UAF entry", async function() {
      ...
      assert.strictEqual(
        entry.hash,
        "06LZxJ5mNuNZj48IZLV816bfp3A7GVtO2O-EeQ1pkTY="
      );
    ...
    it("adds MDS2 UAF entry", async function() {
      ...
      assert.strictEqual(
        entry.hash,
        "iuRviMMnBrXnVrjI0TiacTzKqdG8VXTA6PUy4r7Sxhk="
      );
      // id
      ...
      assert.deepEqual(entry.userVerificationDetails[0], [
        {
          type: "code",
      ...
      assert.deepEqual(entry.userVerificationDetails[1], [
        {
          type: "biometric",
          userVerification: ["fingerprint"],
          blockSlowdown: 0,
