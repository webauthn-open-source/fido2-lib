...
describe("Fido2Result", function() {
  ...
  it("throws if called with new", function() {
    assert.throws(
      ...
      Error,
      "Do not create with 'new' operator. Call 'Fido2AttestationResult.create()' or 'Fido2AssertionResult.create()' instead."
    );
  });
});
...
describe("Fido2AttestationResult", function() {
  ...
  beforeEach(() => {
    ...
    testReq.response.clientDataJSON = h.lib.makeCredentialAttestationNoneResponse.response.clientDataJSON.slice(
      0
    );
    testReq.response.attestationObject = h.lib.makeCredentialAttestationNoneResponse.response.attestationObject.slice(
      0
    );
  });
  ...
  it("passes with 'none' attestation", async function() {
    var ret = await Fido2AttestationResult.create(
  ...
  it("passes with 'u2f' attestation", async function() {
    var ret = await Fido2AttestationResult.create(
      h.lib.makeCredentialAttestationU2fResponse,
      {
        origin: "https://localhost:8443",
        challenge:
          "Vu8uDqnkwOjd83KLj6Scn2BgFNLFbGR7Kq_XJJwQnnatztUR7XIBL7K8uMPCIaQmKw1MCVQ5aazNJFk7NakgqA",
        flags: ["UP", "AT"]
      }
    );
    assert.instanceOf(ret, Fido2AttestationResult);
    ...
    assert.strictEqual(
      ret.audit.info.get("yubico-device-id"),
  ...
  it("passes with Hypersecu u2f attestation", async function() {
    var ret = await Fido2AttestationResult.create(
      h.lib.makeCredentialAttestationHypersecuU2fResponse,
      {
        origin: "https://webauthn.org",
        challenge:
          "pSG9z6Gd5m48WWw9e03AJixbKia0ynEqm7o_9KEkPY0zcaXhjmxoChC5QRnK4E6XIT2QFc_uGycO5lUMygeZgw",
        flags: ["UP", "AT"]
      }
    );
    ...
    assert.strictEqual(
      ret.audit.warning.get("attesation-not-validated"),
      "could not validate attestation because the root attestation certification could not be found"
    );
  ...
  it("passes with Sam's first u2f attestation", async function() {
    var samAnon1 = {
      rawId: coerceToArrayBuffer(
        "85YZwBmkHxXoNdCZvUlUuEAYWDfaMYR7AFeelRdVZEJL6IWJPYozsgutHDm3-go8hnM4tNmrGflVH27Ifixfnw",
        "rawId"
      ),
      id: coerceToArrayBuffer(
        "85YZwBmkHxXoNdCZvUlUuEAYWDfaMYR7AFeelRdVZEJL6IWJPYozsgutHDm3-go8hnM4tNmrGflVH27Ifixfnw",
        "id"
      ),
      response: {
        clientDataJSON: coerceToArrayBuffer(
          "",
          "clientDataJSON"
        ),
        attestationObject: coerceToArrayBuffer(
          "o2NmbXRkbm9uZWdhdHRTdG10oGhhdXRoRGF0YVjElWkIjx7O4yMpVANdvRDXyuORMFonUbVZu4_Xy7IpvdRBAAAAAAAAAAAAAAAAAAAAAAAAAAAAQPOWGcAZpB8V6DXQmb1JVLhAGFg32jGEewBXnpUXVWRCS-iFiT2KM7ILrRw5t_oKPIZzOLTZqxn5VR9uyH4sX5-lAQIDJiABIVgg-wdw0fdf-XYOiCWkXpkGsWQ4rFdD1adtm3T1E9EGBLEiWCBxf3Fc35Z1dDWk9py_IrqcjDofanUESVsZlE5rRfQt3g",
          "attestationObject"
        )
      }
    };
    var samAnon1Challenge =
      "kNWoUtrQ0O2pxKd84IaZOJ-CLJ69eevmV-o8bHcThxgoJ-lsrEZUPhWLWzw4ZJKzZeoBDdNZucIEyUm_4cur2Q";
    await Fido2AttestationResult.create(samAnon1, {
      origin: "https://webauthn.org",
  ...
  it("passes with Sam's second u2f attestation", async function() {
    var ffNonAnon = {
      rawId: coerceToArrayBuffer(
        "3Rt6TThR4PkGcx8UmGoRXji-xvbgoLDlpYgtVdR8uZ2zU3r6lVf8_R9mXvs2d1dDi3p8x1ApIsg5tl6v5beHUA",
        "rawId"
      ),
      id: coerceToArrayBuffer(
        "3Rt6TThR4PkGcx8UmGoRXji-xvbgoLDlpYgtVdR8uZ2zU3r6lVf8_R9mXvs2d1dDi3p8x1ApIsg5tl6v5beHUA",
      ...
      response: {
        clientDataJSON: coerceToArrayBuffer(
          "",
          "clientDataJSON"
        ),
        attestationObject: coerceToArrayBuffer(
          "o2NmbXRoZmlkby11MmZnYXR0U3RtdKJjc2lnWEcwRQIgMKeGa23je6E2jKxsxwoEKF2u0d08ZGLPT-DG-_9nMTzGk6HjrrhgQlWYVD7OIsuX2Unv1dAmqWBpQ0KxS8YRFwKE1SKE1PIpOWacE5SO8BN6-2jbDBqMCIGCSsGAQQBgsQKAgQVMS4zLjYuMS40LjEuNDE0ODIuMS4xMBMGCysGAQQBguUcAgEBBAQDAgUgMCEGCysGAQQBguUcAQEEBBIEEPigEfOMCk0VgAYXER-e3H0wDAYDVR0TAQH_BAIwADANBgkqhkiG9w0BAQsFAAOCAQEAMVxIgOaaUn44Zom9af0KqG9J655OhUVBVW-q0As6AIod3AH5bHb2aDYakeIyyBCnnGMHTJtuekbrHbXYXERIn4aKdkPSKlyGLsA_A-WEi-OAfXrNVfjhrh7iE6xzq0sg4_vVJoywe4eAJx0fS-Dl3axzTTpYl71Nc7p_NX6iCMmdik0pAuYJegBcTckE3AoYEg4K99AM_JaaKIblsbFh8-3LxnemeNf7UwOczaGGvjS6UzGVI0Odf9lKcPIwYhuTxM5CaNMXTZQ7xq4_yTfC3kPWtE4hFT34UJJflZBiLrxG4OsYxkHw_n5vKgmpspB3GfYuYTWhkDKiE8CYtyg87mhhdXRoRGF0YVjElWkIjx7O4yMpVANdvRDXyuORMFonUbVZu4_Xy7IpvdRBAAAAAAAAAAAAAAAAAAAAAAAAAAAAQN0bek04UeD5BnMfFJhqEV44vsb24KCw5aWILVXUfLmds1N6-pVX_P0fZl77NndXQ4t6fMdQKSLIObZer-W3h1ClAQIDJiABIVggNffg6YQ33oZU8wQEBJzQmIRQW-TmDawtPQzMDoV2P0UiWCCGtepcwfyadjsJEAIRxnBtdMCerQ332aWOC_hGgE74-w",
          "attestionObject"
        )
      }
    };
    var ffNonAnonChallenge =
      "QQTc246fjLHnnwNrminCKyJE-Ng3skW30uq4L2vqx_xNdj9ZIa4B3AGhw6f__RijOs6SbbP6m6lkLcMJG4gRYg";
    await Fido2AttestationResult.create(ffNonAnon, {
      origin: "https://webauthn.org",
  ...
  it("passes with 'packed' attestation", async function() {
    var ret = await Fido2AttestationResult.create(
      h.lib.makeCredentialAttestationPackedResponse,
      {
        origin: "https://webauthn.org",
        challenge:
          "uVX88IgRa0SSrMIRT_q7cRcdfgfRBxCgn_pkpUAnXJK2zOb307wd1OLXQ0AuNaMtBR3amk6HYzp-_VxJTPpwGw",
        flags: ["UP", "AT"]
      }
    );
    ...
    assert.isTrue(
      auditInfo.has("subject-key-identifier"),
      "audit info has subject-key-identifier"
    );
    assert.isTrue(
      auditInfo.has("authority-key-identifier"),
      "audit info has authority-key-identifier"
    );
    assert.isTrue(
      auditInfo.has("basic-constraints"),
      "audit info has basic-constraints"
    );
    assert.isTrue(
      auditInfo.has("fido-u2f-transports"),
      "audit info has fido-u2f-transports"
    );
    assert.isTrue(auditInfo.has("fido-aaguid"), "audit info has fido-aaguid");
    assert.isTrue(
      auditInfo.has("attestation-type"),
      "audit info has attestation-type"
    );
    assert.isTrue(
      ret.audit.warning.has("attesation-not-validated"),
      "audit warning has attesation-not-validated"
    );
  });
  ...
  it("passes with 'tpm' attestation", async function() {
    var ret = await Fido2AttestationResult.create(
      h.lib.makeCredentialAttestationTpmResponse,
      {
        origin: "https://webauthn.org",
        challenge:
          "wk6LqEXAMAZpqcTYlY2yor5DjiyI_b1gy9nDOtCB1yGYnm_4WG4Uk24FAr7AxTOFfQMeigkRxOTLZNrLxCvV_Q",
        flags: ["UP", "AT"]
      }
    );
    ...
    assert.isObject(ret.authnrData.get("alg"));
    assert.strictEqual(
      ret.authnrData.get("alg").algName,
    ...
    assert.isTrue(
      auditInfo.has("certificate-policies"),
      "audit info has certificate-policies"
    );
    assert.isTrue(
      auditInfo.has("ext-key-usage"),
      "audit info has ext-key-usage"
    );
    assert.isTrue(
      auditInfo.has("subject-alt-name"),
      "audit info has subject-alt-name"
    );
    assert.isTrue(
      auditInfo.has("authority-key-identifier"),
      "audit info has authority-key-identifier"
    ...
    assert.isTrue(
      auditInfo.has("authority-info-access"),
      "audit info has authority-info-access"
    );
    assert.isTrue(
      auditInfo.has("attestation-type"),
      "audit info has attestation-type"
...
describe("Fido2AssertionResult", function() {
  ...
  it("works with WindowsHello", async function() {
    var ret = await Fido2AssertionResult.create(
